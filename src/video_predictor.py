...
predictor = SAM2VideoPredictor.from_pretrained(model_path)
...
# 
# During initialization, it loads all the JPEG frames in `video_path` and stores their pixels in `` (as shown in the progress bar below).
...
# Note: if you have run any previous tracking using this ``, please reset it first via `reset_state`.
# 
# (The cell below is just for illustration; it's not needed to call `reset_state` here as this `` is just freshly initialized above.)
